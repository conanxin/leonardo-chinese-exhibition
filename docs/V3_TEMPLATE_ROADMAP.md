...
| 项 | 值 |
|---|---|
| 状态 | **Done**（`v3.3-real-template-quality-gate` @ `fce2efb`，tag + GitHub Release + freeze report 全部落地） |
...
| 下一步 | v3.4 Real Second Exhibition Hardening（本 round）或 v3.4-real-stable-freeze |
...
| 起点 | 已封版的 `v3.3-real-template-quality-gate` @ `fce2efb` |
| 交付物 | `_pilots/second-exhibition-pilot/PILOT_QA_CHECKLIST.md` + `PILOT_HANDOFF.md` + `docs/PILOT_QA_REPORT.md`（Playwright 实测），pilot README 追加"Hardening notes"小节 |
| 范围 | pilot hardening 文档 + 本地 render QA 记录；不修改 `_pilots/` 的 `site/`、`data/`、`assets/` 实质内容 |
| 不做的事 | 不修改 live site，不修改 `_template/site/` / `_template/data/`，不修改 `scripts/template_quality_gate.py`，不修改 pilot 的 `site/index.html` / `site/style.css` / `site/script.js` / `data/*.json` / `assets/diagrams/*.svg`（如 quality gate 或 local render 发现真实问题才修改），不创建新 pilot，不部署，不移动任何旧 tag，不修改旧 GitHub Releases |
| 下一步 | v3.4-real-stable-freeze 或 v4.0-real-second-exhibition-plan |

## v3.5 Optional Real Second Exhibition

| 项 | 值 |
|---|---|
| 状态 | **Optional**（只有 v3.4 hardening 完成后才做） |
| 起点 | v3.0 / v3.1 / v3.2 / v3.3 / v3.4 都封版后 |
| 交付物 | 第二个真实展览（独立项目） |
...
v3.2-real-template-documentation  (Done — tag + Release)
    ↓
v3.3-real-template-quality-gate  (Done — script + tag + Release)
    ↓
