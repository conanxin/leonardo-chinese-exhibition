...
## v2.8 Real Deep Content (基于真实 v2.7 tag 重建)

v2.8 是 **从真实 `v2.7-zh-exhibition-polish` tag 重新做**的深度内容扩展。**不**引用之前被记录为 phantom / unverified 的 v2.8 历史，不做模板化、不做 source rights audit、不做双语、不做教育版。

- **基于**: verified `v2.7-zh-exhibition-polish` tag @ `f58f6b4`
- **Content commit**: `71c7403`
- **Live URL**: https://conanxin.github.io/leonardo-chinese-exhibition/
- **Status**: 内容 commit 已完成，stable freeze 留给下一轮
- **不做**: bilingual / education / source audit / template / tag / release

### 本轮新增

- **4 个深度内容模块**：
  - `被装订改变的思想` (`.deep-reading-block`) — 解释手稿命运不是保存史而是关系史
  - `纸页也在说话` (`.material-evidence-block`) — 水印 / 裁切边 / 编号 / 空白与方向作为第一手证据
  - `图像如何成为思考工具` (`.visual-thinking-block`) — 水 / 肌肉 / 动物 / 机械四种图式
  - `从看图到建模` (`.research-model-block`) — 把平台九大工具从列表提升为研究方法论
- **深化 postscript**：从「不是天才崇拜」深化为「关于知识如何生成、材料如何打断、数字平台如何重新连接、今天我们能学到什么」四个层次
- **新增长文**：[`docs/CURATORIAL_ESSAY_ZH.md`](docs/CURATORIAL_ESSAY_ZH.md) — 中文策展长文（2000–3000 字，五节）
- **新增研究笔记**：[`docs/DEEP_RESEARCH_NOTES_ZH.md`](docs/DEEP_RESEARCH_NOTES_ZH.md) — 后续可深挖的问题、需要查证的事实、后续轮内容方向

### 标记 / 不动原则

- v2.8 三层 marker 已加（meta + comment + footer），旧 marker 全部保留
- v2.7 / v2.6 / v2.0 tags 全程未动
- 旧 GitHub Releases 未触碰
- `site/script.js` 未修改（深度模块用 `aside` 实现，不进入 `getTourSections()` 的 section registry）
- `_template/` / `_pilots/` 未创建
- posts / case-study / release-assets 既有文件未触碰

详见 [`reports/leonardo_chinese_exhibition_v2_8_real_deep_content_report.md`](reports/leonardo_chinese_exhibition_v2_8_real_deep_content_report.md)。

## v2.8 Real Deep Content (verified, 2026-07-07 freeze)

v2.8 Real Deep Content 在 [content commit `18b551b`](https://github.com/conanxin/leonardo-chinese-exhibition/commit/18b551b) 之上做 **真实稳定封版**，从 verified `v2.7-zh-exhibition-polish` tag 重建，**不**引用任何 phantom v2.8 / v2.9 / v3.x 历史。

- **Live**: https://conanxin.github.io/leonardo-chinese-exhibition/
- **Tag**: `v2.8-real-deep-content`（freeze round 新增）
- **Source tag**: `v2.7-zh-exhibition-polish` @ `f58f6b4`
- **Verified live byte size**: 92,507 B
- **Status**: verified real deep content（v2.9 / v3.x 推迟到从 verified state 重建之后）

主要内容：
- 4 个深度模块（`.deep-reading-block` / `.material-evidence-block` / `.visual-thinking-block` / `.research-model-block`）— 各 1 个，全部用 `<aside>` 实现以避免修改 `site/script.js`
- 深化 postscript（约 480 字）
- 中文策展长文 [`docs/CURATORIAL_ESSAY_ZH.md`](docs/CURATORIAL_ESSAY_ZH.md)
- 研究深化笔记 [`docs/DEEP_RESEARCH_NOTES_ZH.md`](docs/DEEP_RESEARCH_NOTES_ZH.md)
- 保留 v2.5-real guided accessibility、v2.6 stable 交互、v2.7 中文体验精修全部能力（runtime section-nav 31 links 不变，Playwright 验证 0 console errors）
...
## v2.9 Real Source & Rights Audit (verified, 2026-07-07 freeze)

v2.9 Real Source & Rights Audit 在 [audit commit `dbcc563`](https://github.com/conanxin/leonardo-chinese-exhibition/commit/dbcc563) 之上做 **真实稳定封版**，从 verified `v2.8-real-deep-content` tag 重建，**不**引用任何 phantom v2.9 / v3.x 历史。

- **Live**: https://conanxin.github.io/leonardo-chinese-exhibition/
- **Tag**: `v2.9-real-source-rights-audit`（freeze round 新增）
- **Source tag**: `v2.8-real-deep-content` @ `65b4fbc`
- **Verified live byte size**: 92,976 B
- **Status**: verified source and rights audit（v3.x 推迟到从 verified state 重建之后）

主要审计结果：
- **Collection / manuscript images**: 6 张（温莎 4 + Codex Atlanticus 2）— 全部 Wikimedia Commons 公共域授权，RCIN 编号与 Wikimedia 文件名嵌入编号一致
- **Platform screenshots**: 5 张（3 引用 + 2 unused）— 引用图均含 source-note + credit-line；unused 标记 follow-up
- **Self-made SVG**: 7 张 — 全部以「示意图 · 作者绘制」+ 依据标注，明确 project-generated
- **Site metadata**: 2 张 — favicon / og-cover

来源 / 权利文档：
- [`docs/SOURCE_AUDIT_MANIFEST.md`](docs/SOURCE_AUDIT_MANIFEST.md) — 详细清单
- [`docs/RIGHTS_AND_SOURCES.md`](docs/RIGHTS_AND_SOURCES.md) — rights 综述 + reuse caution
- [`research/image-candidates.md`](research/image-candidates.md) — audit 状态 + confidence + follow-up items
...
- 状态：v3.0 real template extraction audit 已 stable freeze（**PASS**）
- 起点：`v2.9-real-source-rights-audit` @ `a1e667e`
- 实测 live byte size：92,976 B（v3.0 round + freeze round live site 均未修改）
- 交付物：`_template/` 真实创建（17 个文件；实测 `find _template -type f | wc -l`）
- 范围：模板骨架 + 数据示例 + 站点骨架 + 文档模板 + 提取审计 + 可复用组件表 + 路线图
- 不做的事：
  - 不修改 live `site/index.html` / `style.css` / `script.js`
  - 不创建 `_pilots/`（v3.1 才会创建第一个 pilot）
  - 不部署 `_template/site/`（与 live 物理隔离）
...
- [`docs/RELEASE_NOTES_v3.0_REAL_TEMPLATE_EXTRACTION_AUDIT.md`](docs/RELEASE_NOTES_v3.0_REAL_TEMPLATE_EXTRACTION_AUDIT.md) — release notes
...
- **状态**：v3.1 real second exhibition pilot 已 stable freeze（**PASS**）；repository pilot，**不部署**到 GitHub Pages
...
- **不做的事**：不修改 `_template/`，不部署 pilot，不引用任何真实馆藏图，不创建第二个 pilot
- **下一步**：v3.2 template documentation（写完整使用手册）
...
- [`docs/RELEASE_NOTES_v3.1_REAL_SECOND_EXHIBITION_PILOT.md`](docs/RELEASE_NOTES_v3.1_REAL_SECOND_EXHIBITION_PILOT.md) — release notes
...
- **Status**: verified template quality gate（PASS）
- **Verified live byte size**: 92,976 B
- **Source tag**: `v3.2-real-template-documentation`
- **Quality gate**:
  - [`scripts/template_quality_gate.py`](scripts/template_quality_gate.py)
  - 37/37 checks PASS
- **Notes**:
  - Does not modify live site
  - Does not modify pilot
  - Keeps template and pilot quality checks repeatable
...
## v5.6c Second Exhibition v0.2 Production (verified, 2026-07-12)

- Public version: `second-exhibition-v0.2`
- Public URL: https://conanxin.github.io/leonardo-chinese-exhibition/second-exhibition/
- Root: 92,976 B / SHA `e2be1077fa7e601d50e300f7c98ddc19f802b1c38260c5e18e4763c2a1963afc`
- Second exhibition: 31,452 B / SHA `00894e8dfa0fa1e40ed3df803afa0036a2a070bee8f42cdfb636cd31d68b3aa2`
- Sections: 4 / artifact cards: 6 / glossary: 14 / images: 6 / 6 unchanged
- Default production healthcheck validates v0.2 directly (no candidate flag)
- Public 5-viewport browser QA: 5/5 PASS (1440×1000, 1280×900, 768×1024, 390×844, 320×700)
  - 0 console / page / failed / external errors, 0 overflow
  - All 8 interactions PASS
